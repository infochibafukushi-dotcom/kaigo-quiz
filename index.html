...
async function applyEditorToQuestion(q) { q.question = norm(document.getElementById("eq-question")?.value); q.explanation = norm(document.getElementById("eq-exp")?.value); q.imageData = q.imageData || ""; syncPreviewToQuestion(q); const imageInput = document.getElementById("eq-image-file"); const selectedImage = imageInput?.files?.[0]; if (selectedImage) { q.imageData = await readFileAsDataURL(selectedImage); } if (q.type === "ox" || q.type === "choice" || q.type === "fill") q.answer = norm(document.getElementById("eq-answer")?.value); if (q.type === "choice" || q.type === "multi") q.choices = (document.getElementById("eq-choices")?.value || "").split("\n").map(norm).filter(Boolean); if (q.type === "multi") q.answers = (document.getElementById("eq-answers-text")?.value || "").split("\n").map(norm).filter(Boolean); if (isMB(q.type)) { q.blankCount = Math.max(1, Number(document.getElementById("eq-blankCount")?.value) || 1); q.answers = [...document.querySelectorAll(".edit-answer")].map((x) => norm(x.value)).slice(0, q.blankCount); while (q.answers.length < q.blankCount) q.answers.push(""); if (q.answers.length !== q.blankCount) throw new Error("blankCount mismatch"); } }
...
async function ensureUnitWithIds(course, unit) {
  ...
  if (currentId !== undefined && currentId !== null && currentId !== "") return unit;
  const courseTitle = norm(course?.title);
  const unitTitle = norm(unit?.title);
  ...
  const created = res?.unit || {};
  const resolvedId = created.unitId ?? created.id ?? null;
  return {
    ...unit,
    id: resolvedId,
    unitId: resolvedId,
...
async function saveQuestion(qi) { if (saving) return; const list = curUnit().questions || []; const q = (editingQuestionId != null ? list.find((x) => String(x?.id) === String(editingQuestionId)) : null) || list[qi]; if (!q) { alert("保存対象の問題が見つかりません。"); return; } const fixedType = q.type; const fixedBlankCount = q.blankCount; const fixedAnswers = Array.isArray(q.answers) ? q.answers.slice() : []; try { await applyEditorToQuestion(q); } catch { alert("blankCount と answers 数が一致しないため保存できません。"); return; } if (editingQuestionId != null && q.type !== fixedType) { q.type = fixedType; if (isMB(fixedType)) { q.blankCount = fixedBlankCount; q.answers = fixedAnswers; } } saving = true; try { const course = db.courses[courseIndex] || {}; const selectedCourse = { id: course?.id ?? course?.courseId ?? null, courseId: course?.courseId ?? course?.id ?? null, title: course?.title }; const ensuredUnit = await ensureUnitWithIds(course, course.units?.[unitIndex] || {}); if (course.units?.[unitIndex]) course.units[unitIndex] = ensuredUnit; const selectedUnit = { id: ensuredUnit?.id ?? ensuredUnit?.unitId ?? null, unitId: ensuredUnit?.unitId ?? ensuredUnit?.id ?? null, title: ensuredUnit?.title }; let resolvedCourseId = selectedCourse?.courseId || selectedCourse?.id || q?.courseId; let resolvedUnitId = selectedUnit?.unitId || selectedUnit?.id || q?.unitId; console.log("SELECTED COURSE", selectedCourse); console.log("SELECTED UNIT", selectedUnit); console.log("SELECTED COURSE FULL", JSON.stringify(selectedCourse, null, 2)); console.log("SELECTED UNIT FULL", JSON.stringify(selectedUnit, null, 2)); console.log("RESOLVED IDS", { courseId: selectedCourse?.courseId, courseId2: selectedCourse?.id, unitId: selectedUnit?.unitId, unitId2: selectedUnit?.id }); if (!resolvedCourseId || !resolvedUnitId) { console.error("SAVE BLOCKED: unit id missing", selectedUnit); alert("保存先単元IDがありません"); return; } q.courseId = resolvedCourseId; q.unitId = resolvedUnitId; const payload = buildQuestionPayload(q, { courseId: resolvedCourseId, unitId: resolvedUnitId }); if (payload.courseId === undefined || payload.courseId === null || payload.courseId === "" || payload.unitId === undefined || payload.unitId === null || payload.unitId === "") { throw new Error("新規問題の保存先(courseId/unitId)が不正です。"); } console.log("FINAL PAYLOAD", payload); const path = q.id ? `/api/questions/${q.id}` : `/api/questions`; const method = q.id ? "PUT" : "POST"; const res = await fetch(`${API_BASE}${path}`, { method, headers: { "content-type": "application/json" }, body: JSON.stringify(payload) }); const text = await res.text(); if (!res.ok) { console.error(text); throw new Error(`${res.status} ${res.statusText}`); } if (text) { try { JSON.parse(text); } catch (_) { } } await loadData(true); renderAdmin(); } catch (e) { alert(`保存エラー: ${e.message}`); } finally { saving = false; } }
async function createQuestion(type) { const q = nQ({ type }); try { await applyEditorToQuestion(q); } catch { alert("blankCount と answers 数が一致しないため保存できません。"); return; } curUnit().questions.push(q); await saveQuestion(curUnit().questions.length - 1); }
...
else if (a === "add-unit") { const n = prompt("単元名"); const course = db.courses[courseIndex]; if (n && course?.title) { try { const res = await api("/api/units", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ course: norm(course.title), title: norm(n) }) }); const created = res?.unit || {}; await loadData(true); const targetCourseIndex = db.courses.findIndex((x) => String(x?.courseId ?? x?.id ?? "") === String(created.courseId ?? "")); courseIndex = targetCourseIndex >= 0 ? targetCourseIndex : courseIndex; const units = db.courses[courseIndex]?.units || []; const targetUnitIndex = units.findIndex((x) => String(x?.unitId ?? x?.id ?? "") === String(created.unitId ?? created.id ?? "")); unitIndex = targetUnitIndex >= 0 ? targetUnitIndex : Math.max(0, units.length - 1); } catch (e) { alert(`単元追加エラー: ${e.message}`); } } renderAdmin(); } else if (a === "ren-unit") { const u = curUnit(); if (!u) return; const n = prompt("単元名", u.title); if (n != null) u.title = n; renderAdmin(); } else if (a === "toggle-unit") { const u = curUnit(); if (u) { u.isVisible = u.isVisible === false ? true : false; if (u.id) { try { await api(`/api/units/${u.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ isVisible: u.isVisible }) }); } catch (_) { } } renderAdmin(); } } else if (a === "del-unit") { if (confirm("単元削除しますか？")) { db.courses[courseIndex].units.splice(unitIndex, 1); unitIndex = 0; renderAdmin(); } }
else if (a === "save-all") alert("各問題の保存で反映されます。保存対象の問題を編集画面から保存してください。"); else if (a === "export-json") { const blob = new Blob([JSON.stringify(db, null, 2)], { type: "application/json" }); const ael = document.createElement('a'); ael.href = URL.createObjectURL(blob); ael.download = 'questions.json'; ael.click(); URL.revokeObjectURL(ael.href); } else if (a === "img-clear") { const p = document.getElementById('img-preview'); if (p) p.innerHTML = ''; const fi = document.getElementById('eq-image-file'); if (fi) fi.value = ''; }
});
