...
const eDB=(d)=>({appTitle:d?.appTitle||"カイゴクイズ",courses:Array.isArray(d?.courses)?d.courses.map((c)=>{
  console.log("RAW COURSE",c);
  const courseId=c?.courseId??c?.id??null;
  if(courseId==null||courseId===""){console.error("API response missing IDs",{scope:"course",course:c});throw new Error("API response missing IDs");}
  const units=Array.isArray(c?.units)?c.units.map((u)=>{
    console.log("RAW UNIT",u);
    const unitId=u?.unitId??u?.id??null;
    if(unitId==null||unitId===""){console.error("API response missing IDs",{scope:"unit",course:c,unit:u});throw new Error("API response missing IDs");}
    const questions=Array.isArray(u?.questions)?u.questions.map(nQ):[];
    questions.forEach((q)=>{if(q.unitId==null||q.unitId==="")q.unitId=unitId;if(q.courseId==null||q.courseId==="")q.courseId=courseId;});
    return {id:unitId,unitId,title:u?.title||"",isVisible:u?.isVisible!==false,questions};
  }):[];
  return {id:courseId,courseId,title:c?.title||"",units};
}):[]});
