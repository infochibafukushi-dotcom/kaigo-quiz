...
const eDB=(d)=>({appTitle:d?.appTitle||"カイゴクイズ",courses:Array.isArray(d?.courses)?d.courses.map(c=>({id:c?.id,courseId:c?.courseId,title:c?.title||"",units:Array.isArray(c?.units)?c.units.map(u=>({id:u?.id,unitId:u?.unitId,title:u?.title||"",isVisible:u?.isVisible!==false,questions:Array.isArray(u?.questions)?u.questions.map(nQ):[]})):[]})):[]});
