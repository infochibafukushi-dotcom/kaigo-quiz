...
const eDB=(d)=>({appTitle:d?.appTitle||"カイゴクイズ",courses:Array.isArray(d?.courses)?d.courses.map((c)=>{const units=Array.isArray(c?.units)?c.units.map((u)=>{const questions=Array.isArray(u?.questions)?u.questions.map(nQ):[];const seed=questions.find(q=>q?.courseId!=null&&q?.courseId!==""&&q?.unitId!=null&&q?.unitId!=="");const unitId=u?.id??u?.unitId??seed?.unitId??null;questions.forEach((q)=>{if((q.unitId==null||q.unitId==="")&&unitId!=null&&unitId!=="")q.unitId=unitId;if((q.courseId==null||q.courseId==="")&&seed?.courseId!=null&&seed?.courseId!=="")q.courseId=seed.courseId;});return {id:unitId,unitId,title:u?.title||"",isVisible:u?.isVisible!==false,questions};}):[];const courseSeed=units.flatMap(u=>u.questions||[]).find(q=>q?.courseId!=null&&q?.courseId!=="");const courseId=c?.id??c?.courseId??courseSeed?.courseId??null;units.forEach((u)=>{(u.questions||[]).forEach((q)=>{if((q.courseId==null||q.courseId==="")&&courseId!=null&&courseId!=="")q.courseId=courseId;if((q.unitId==null||q.unitId==="")&&u.unitId!=null&&u.unitId!=="")q.unitId=u.unitId;});});return {id:courseId,courseId,title:c?.title||"",units};}):[]});
