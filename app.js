...
const eDB=(d)=>({appTitle:d?.appTitle||"カイゴクイズ",courses:Array.isArray(d?.courses)?d.courses.map((c,ci)=>{const generatedStableCourseId=stableCourseId(c,ci);const units=Array.isArray(c?.units)?c.units.map((u,ui)=>{const questions=Array.isArray(u?.questions)?u.questions.map(nQ):[];const generatedStableUnitId=stableUnitId(u,c,ci,ui);const unitId=u?.id??u?.unitId??questions.find(q=>q?.unitId!=null)?.unitId??generatedStableUnitId;questions.forEach((q)=>{if(q.unitId==null||q.unitId==="")q.unitId=unitId;});return {id:unitId,unitId,title:u?.title||"",isVisible:u?.isVisible!==false,questions};}):[];const courseId=c?.id??c?.courseId??units.flatMap(u=>u.questions||[]).find(q=>q?.courseId!=null)?.courseId??generatedStableCourseId;units.forEach((u)=>{(u.questions||[]).forEach((q)=>{if(q.courseId==null||q.courseId==="")q.courseId=courseId;});});return {id:courseId,courseId,title:c?.title||"",units};}):[]});
...
const stableCourseId=(course,ci)=>`course-${course?.slug||""}-${course?.title||""}-${ci}`;
const stableUnitId=(unit,course,ci,ui)=>`unit-${course?.slug||""}-${course?.title||""}-${unit?.slug||""}-${unit?.title||""}-${ci}-${ui}`;
